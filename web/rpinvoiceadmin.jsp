...
<sql:transaction dataSource="${pssdb}">
    ...
    <sql:query var="r">
        SELECT CONCAT(CAST(saleproduct.num as char), '. ', saleproduct.name) as pname, 
            ...
            @totaldel * saleproduct.unitprice * (1 - saleproduct.profit) as due
            FROM saleproduct
            ...
            LEFT JOIN (SELECT saleproduct.id AS id, SUM(flatsdelivered * unitsperflat) AS unitsdelivered
...
<c:set var="columnNames" value="Product,Unit Price,Pre-Sold,Units Received,Amount Due"/>
<c:set var="columnTypes" value="text,money,number,number,money"/>
